...
(function ($) {
  ...
  $olMenuToggleBtn.on("click", function () {
    ...
    if ($("body").hasClass("tt-ol-menu-open")) {
      ...
      tl_olMenuOut.set(".tt-ol-menu-list > li, .tt-ol-menu-anim", {
        clearProps: "all",
      }); // clearProps only
      ...
      if ($ttTestimonialsSlider.hasClass("ts-scale-down")) {
        $ttTestimonialsSlider
          ...
          .on("mousedown touchstart pointerdown", function (e) {
            if (e.which === 1) {
              // Affects the left mouse button only!
              gsap.to($ttTestimonialsSlider.find(".swiper-slide"), {
                duration: 0.7,
                scale: 0.9,
              });
            }
  ...
  if ($("#page-header").hasClass("ph-content-parallax")) {
    ...
    if ($("#page-header .project-info-list").length) {
      $("#page-header .project-info-list > ul > li").wrapInner(
  ...
  var news_btn = document.querySelector("#news-btn");
  var news_input = document.querySelector("#news-input");
  var news_response = document.querySelector("#news-response");

  news_btn.addEventListener("click", showMessage);

  function showMessage() {
    if (news_input.value !== "" && news_input.value.match(/\S+@\S+\.\S+/)) {
      news_response.appendChild(
        document.createTextNode("Thanks, you have subscribed to our Newsletter")
      );
    } else {
      window;
    }
  ...
  const questions = [
    ...
    { question: "Explain Your Project", type: "text" },
  ];
  ...
  function getQuestion() {
    ...
    inputLabel.innerHTML = questions[position].question.toUpperCase();
    // Get Current Type
